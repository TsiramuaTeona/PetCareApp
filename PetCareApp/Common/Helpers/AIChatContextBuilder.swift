...
struct AIChatContextBuilder {
    ...
    static func build(pets: [Pet], logsByPetId: [String: [HealthLog]]) -> String {
        ...
        for pet in pets {
            ...
            if let breed = pet.breed, !breed.isEmpty { lines.append("- Breed: \(breed)") }
            if let color = pet.color, !color.isEmpty { lines.append("- Color: \(color)") }
        ...
        lines.append("")
        lines.append("If user asks medical advice, ask clarifying questions: species, age, weight, duration, symptoms.")
    ...
    private static func summarizeHealth(logs: [HealthLog]) -> [String] {
        ...
        if let lastWeight = logs
           ...
           let value = lastWeight.value {
            out.append("- Latest weight: \(String(format: "%.1f", value)) kg on \(lastWeight.date.formatted(.dateTime.year().month().day()))")
        }
        ...
        if !activeMeds.isEmpty {
            ...
            for med in activeMeds.prefix(5) {
                let due = med.nextDueDate?.formatted(.dateTime.year().month().day().hour().minute()) ?? "unknown"
                let dose = (med.dosage?.isEmpty == false) ? " (\(med.dosage!))" : ""
                out.append("  - \(med.title)\(dose) – next due: \(due)")
            }
        }
        
        let upcoming = logs
            .filter { $0.category != .medication && !$0.isResolved && $0.nextDueDate != nil }
            .sorted { ($0.nextDueDate ?? .distantFuture) < ($1.nextDueDate ?? .distantFuture) }
        
        if !upcoming.isEmpty {
            out.append("- Upcoming health items:")
            for item in upcoming.prefix(5) {
                let due = item.nextDueDate?.formatted(.dateTime.year().month().day()) ?? "unknown"
                out.append("  - \(item.category.rawValue): \(item.title) – due: \(due)")
            }
        }
